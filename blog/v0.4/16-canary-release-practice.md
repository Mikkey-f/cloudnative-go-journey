...
在完成第 12、13 篇对 Ingress 与 Istio 的学习之后，我们终于要把“流量治理”落到发布流程上。真实生产环境中，直接把 v1 一键替换为 v2 往往意味着：

- ❌ 线上突发 Bug 谁来兜底？
- ❌ 响应时间飙升怎么办？
- ❌ 全量用户都踩坑还能回滚吗？

为了解决这些隐患，我在 v0.4 项目里引入了 **金丝雀发布（Canary Release）**：用极小的流量验证新版本，当一切稳定再逐步放量，直到最终全量切换。本文会按照“实战 + 决策”的节奏展开，带你完成一次完整的 Istio 金丝雀发布：

- ✅ 先理解金丝雀发布的价值和与其他策略的区别
- ✅ 准备两个版本的应用与配置，明确差异点
- ✅ 使用 Istio VirtualService/DestinationRule 实现 90/10 分流
- ✅ 编写脚本 + Prometheus 查询验证流量
- ✅ 设计分阶段放量/回滚流程与最佳实践

读完后，你可以复刻一套可回滚、可监控、可扩展的发布流程。
...
这篇文章完整演示了使用 Istio 实现金丝雀发布：

**核心要点**：
1. 🐤 **金丝雀发布**：逐步放量，风险可控，随时可回滚
2. ⚖️ **流量分割**：VirtualService 的 weight 配置实现精确的流量分配
3. 🎯 **子集定义**：DestinationRule 通过标签区分 v1 和 v2
4. 📊 **流量验证**：通过脚本和监控确认流量分配符合预期
5. 🚀 **逐步迁移**：10% → 50% → 100%，充分验证每个阶段

**实战成果**：
- ✅ 成功部署 v1 和 v2 两个版本
- ✅ 配置 90/10 流量分割
- ✅ 验证流量分配准确（88%-92% v1，8%-12% v2）
- ✅ 掌握流量策略调整方法
...
**下一步学习**：
- 探索基于 Header 的流量路由（特定用户访问 v2）
- 学习故障注入测试（模拟 v2 异常）
- 深入 Istio 的可观测性功能（Jaeger、Kiali）

---

**v0.4 系列完结**：
- 第11篇：《从 Service 到 Ingress：K8s 服务暴露完全指南》
- 第12篇：《Ingress Controller 实战：Nginx Ingress 深度解析》
- 第13篇：《初探服务网格：Istio 让微服务更简单》
- 第14篇：《金丝雀发布实战：灰度上线新版本》（本篇）
